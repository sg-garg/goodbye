...
<hr/>
		...
		<form action="point-of-contact.php" >
		<input type="submit" value="New Point of Contact" />
		</form>
...
<?php
	...
	$sqlppoc = "SELECT * FROM pocontact WHERE userid = '$userid' AND prim = '1' ORDER BY recipient_ln ASC LIMIT 3";
...
echo '<th width="25%">Name</th><th width="25%">Relationship</th><th width="25%">Date Added</th><th width="25%">&nbsp;</th>';
	while($rowppoc = $qppoc->fetch()) {
		echo '<tr><td class="tw300">'. $rowppoc['recipient_fn']. ' ' .$rowppoc['recipient_mi']. '  ' . $rowppoc['recipient_ln'].'</td><td>'. $rowppoc['relation'].'</td><td><a href="update-poc.php?poc_id='.$rowppoc[poc_id].'">Edit</a></td></tr>';
		}
echo '</table>';
...
<?php 
	...
	$sqlpoc = "SELECT * FROM pocontact WHERE userid='$userid' ORDER BY recipient_ln ASC LIMIT 3";
...
<!-- GOODBYE -->
<form action="compose-goodbye.php" >
<input type="submit" value="Compose a New Goodbye Message" />
</form>
<?php 
...
echo '<table cellpadding="5" width="560">';
echo '<tr><th width="25%">Name</th><th width="25%">Date</th><th width="25%">Relationship</th><th width="25%">Edit</th></tr>';
		while($row = $q->fetch()){
		//check which type (1 or 2)
if ($row['type'] == 1){$typeofmess = "E-mail";} else {$typeofmess = "USPS Mail";}
		echo '<tr><td>'. $row['recipient_fn'].' '.$row['recipient_mi'].' '.$row['recipient_ln'].'</td><td>'. $row['msg_date'].' </td><td>'. $row['relation'].'</td><td><a href="update-goodbye.php?msg_id='.$row['msg_id'].'">Edit</a> ('.$typeofmess.')</td></tr>';
		}
echo '</table>';
...
<ul class="inline">
	<li class="inline">
	<form action="video-upload-s.php" ><input type="submit" value="Upload a Video" /></form></li>
	<li class="inline"> or </li>
	<li class="inline">
	<form action="https://www.egoodbyes.com/rec/index.php" >
	<input type="submit" value="Create a Video" /></form>
	</li>
</ul>
...
echo "<tr><th width=\"25%\">Title</th><th width=\"25%\">Date</th><th width=\"25%\">Recipient</th><th width=\"25%\">&nbsp;</th></tr>";
...
*/
    ...
    <td>$textdate</td>
    <td><i>$vrecipient</i></td><td><a href='video-dbupdate.php?v=$path2'>Edit</a></td></tr>\n";
...
echo '<table cellpadding=\"5\" width=\"560\">';
echo '<tr><th width=\"25%\">Name</th><th width=\"25%\">Edit</th><th width=\"25%\">Upload Images</th><td width=\"25%\">&nbsp;</td>';

$sqlm1 = "SELECT * FROM memorials WHERE userid = '$userid' LIMIT 3 ";
...
<td><a href='upload-images-mem.php?mem_id=" . $rowm1['mem_id'] . "'>Upload Images for this Memorial</a></td><td>&nbsp;</td></tr>";
